...
cdf_cpmgee <- function(mod, cont_y, new.data, at.y=0, se=TRUE){
  if(mod$corr.mod == 'independence'){
    return(cdf_orm(mod, cont_y, new.data, at.y, se=TRUE))
  }else{
    return(cdf_repolr(mod, cont_y, new.data, at.y, se=TRUE))
  }
}
...
quantile_cpmgee <- function(mod, cont_y, new.data, probs=0.5, se=TRUE){
  if(mod$corr.mod == 'independence'){
    return(quantile_orm(mod, cont_y, new.data, probs, se=TRUE))
  }else{
    return(quantile_repolr(mod, cont_y, new.data, probs, se=TRUE))
  }
}
...
mean_cpmgee <- function(mod, cont_y, new.data, se=TRUE){
  if(mod$corr.mod == 'independence'){
    return(mean_orm(mod, cont_y, new.data, se=TRUE))
  }else{
    return(mean_repolr(mod, cont_y, new.data, se=TRUE))
  }
}


#' Calculate CDFs conditional on covariates (exchangeable/AR1 working correlation structure)
#'
#' This functions calculates the CDFs conditional on covariates based on the fitted model
#'  (exchangeable or AR1 working correlation structure) and new data.
#'
#' @param mod the model
#' @param cont_y the continuous response data
#' @param new.data the new data
...
#' @return \item{lb}{a vector of estimated lower bounds of 95\% confidence intervals}
#' @return \item{ub}{a vector of estimated upper bounds of 95\% confidence intervals}
cdf_repolr <- function(mod, cont_y, new.data, at.y=0, se=TRUE){
...
#' Calculate quantiles conditional on covariates (exchangeable/AR1 working correlation structure)
#'
#' This functions calculates the quantiles conditional on covariates based on the fitted model
#' (with exchangeable/AR1 working correlation structure) and new data.
#'
#' @param mod the model
#' @param cont_y the continuous response data
#' @param new.data the new data
...
#' @return \item{lb}{a vector of estimated lower bounds of 95\% confidence intervals}
#' @return \item{ub}{a vector of estimated upper bounds of 95\% confidence intervals}
quantile_repolr <- function(mod, cont_y, new.data, probs=0.5, se=TRUE){
  # inputs
  coef <- mod$coefficients
  cov.par <- mod$robust.var
  order.y <- sort(unique(cont_y))
...
#' Calculate expectations conditional on covariates (exchangeable/AR1 working correlation structure)
#'
#' This functions calculates the expectations conditional on covariates based on the fitted model
#' (with exchangeable/AR1 working correlation structure) and new data.
#'
...
#' @return \item{lb}{a vector of estimated lower bounds of 95\% confidence intervals}
#' @return \item{ub}{a vector of estimated upper bounds of 95\% confidence intervals}
mean_repolr <- function(mod, cont_y, new.data, se=TRUE){
  # inputs
  coef <- mod$coefficients
  cov.par <- mod$robust.var
  order.y <- sort(unique(cont_y))
...
#' Calculate CDFs conditional on covariates (independence working correlation structure)
#'
#' This functions calculates the CDFs conditional on covariates based on the fitted model
#'  (independence working correlation structure) and new data.
#'
#' @param mod the model
#' @param cont_y the continuous response data
#' @param new.data the new data
...
#' @return \item{lb}{a vector of estimated lower bounds of 95\% confidence intervals}
#' @return \item{ub}{a vector of estimated upper bounds of 95\% confidence intervals}
cdf_orm <- function(mod, cont_y, new.data, at.y=0, se=TRUE){
  # logit link
  cumprob <- function(x) 1 / (1 + exp(-x))
  ...
  order.y <- sort(unique(cont_y))
  cov.par <- mod$robust.var
  kint <- length(order.y) - 1
  p <- length(coef) - length(order.y)  + 1
  ...
  if(se){
    ...
    ci.lb <- sapply(1:length(at.y), FUN=function(i){cumprob(lb[, i] - qnorm(0.975)*lb.se[, i])})
    ci.ub <- sapply(1:length(at.y), FUN=function(i){cumprob(lb[, i] + qnorm(0.975)*lb.se[, i])})
...
#' Calculate conditional quantiles (independence working correlation structure)
#'
#' This functions calculates the conditional weighted quantiles based on the fitted model
#' (wth independence working correlation structure) and new data.
#'
#' @param mod the model
#' @param cont_y the continuous response data
#' @param new.data the new data
...
#' @return \item{lb}{a vector of estimated lower bounds of 95\% confidence intervals}
#' @return \item{ub}{a vector of estimated upper bounds of 95\% confidence intervals}
quantile_orm <- function(mod, cont_y, new.data, probs=0.5, se=TRUE){
  # logit link
  cumprob <- function(x) 1 / (1 + exp(-x))
  
  coef <- mod$coefficients
  order.y <- sort(unique(cont_y))
  cov.par <- cov.par <- mod$robust.var
  kint <- length(order.y) - 1
  p <- length(order.y) - length(coef) + 1
...
#' Calculate expectations conditional on covariates (independence working correlation structure)
#'
#' This functions calculates the expectations conditional on covariates based on the fitted model
#' (with independence working correlation structure) and new data.
#'
#' @param mod the model
#' @param cont_y the continuous response data
#' @param new.data the new data
...
#' @return \item{lb}{a vector of estimated lower bounds of 95\% confidence intervals}
#' @return \item{ub}{a vector of estimated upper bounds of 95\% confidence intervals}
mean_orm <- function(mod, cont_y, new.data, se=TRUE){
  # logit link 
  cumprob <- function(x) 1 / (1 + exp(-x))
  ...
  if(se){
    ...
    dmean.dbeta <- apply(dmean.dalpha, 1, sum) * as.matrix(new.data)
    dmean.dtheta <- cbind(dmean.dalpha, dmean.dbeta)
    mean.var <-diag(dmean.dtheta %*% mod$robust.var %*% t(dmean.dtheta))
    mean.se <- sqrt(mean.var)
    result <- cbind(m.mean, mean.se)
    ci <- t(apply(result, 1, FUN=function(x) c(x[1]- qnorm(0.975)*x[2],
                                               x[1]+ qnorm(0.975)*x[2])))
